/*
    add new function: according to the moisture level print some texts on the Lcd and set a red led to remind the ower of the plant when the water level is too low
    icd:import library
        initalize
        textReminder()-define moisture leve
                      -different text output under each level
    led:define redled
        m_Level()-if leve<#, turn on the led
...
//8.specifing the usage of redled, when the value level<=2,it means humidity level is too low.
void m_level(){
    if(level<=2){
...
void setup() {
  ...
  //3.initialize the lcd inside setup()
  lcd.init();                      
  lcd.backlight();
  lcd_clear();
  lcd.setCursor(0, 0);
  lcd.print("Plant humidity");
  lcd.setCursor(0, 1);
  lcd.print("monitor");
  delay(4000);
  // start DHT sensor
...
void loop() {
  ...
  client.loop();
// 4.add textReminder in loop
  textReminder();
}


//5.write textReminder outside the loop
void textReminder(){
  //read the value
  float reading = analogRead(soilPin);
